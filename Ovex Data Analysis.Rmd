...
ggqqplot(alpha, 'faith_pd', facet.by = 'Treatment')
...
lmm_even = lme(pielou_evenness ~ Treatment+Time, random = ~1| Actual.Contents, data = alpha)
summary(lmm_even)
Anova(lmm_even, type = "II")
...
Line Plots of Schaedleri Species

```{r}
line_plots = data.frame(t(bacteria_tx))

#Selecting only Schaedleri species that are statistically significant
line_plots = subset(line_plots, select = c(X8a308a269f4a15e0885ddfba3d4c408e, X8ccde3af6976e7fe31372774390a11af
, ae67c6666749ae89e0b39c620a0c7f21, d2680eca44a23956fabe78a6d85395d8, X2e59f1b99fab85cb63bce2f35a5ca838, X626d5d8df116add83e6b14c60adbc2dc, X64547667c88735701e168289a2a66718
))
...
schaed_1 = ggplot(data = mean_var, aes(x=Time, y=ae67c6666749ae89e0b39c620a0c7f21, group=Treatment))+
  geom_line(aes(color=Treatment))+
  geom_point(aes(color=Treatment))+
  ylim(-1,7.5)+
  ylab("Average CLR Abundance (M.Schaedleri)")+
  xlab("Time (Week)")
schaed_1 %>%
  ggexport(filename = "schaed1.pdf")
  

schaed_2 = ggplot(data = mean_var, aes(x=Time, y=d2680eca44a23956fabe78a6d85395d8, group=Treatment))+
  geom_line(aes(color=Treatment))+
  geom_point(aes(color=Treatment))+
  ylim(-1,7.5)+
  ylab("Average CLR Abundance (M.Schaedleri)")+
...
schaed_3 = ggplot(data = mean_var, aes(x=Time, y=X8a308a269f4a15e0885ddfba3d4c408e, group=Treatment))+
  ...
  xlab("Time (Week)")
schaed_3 %>%
  ggexport(filename = "schaed3.pdf")

schaed_4 = ggplot(data = mean_var, aes(x=Time, y=X8ccde3af6976e7fe31372774390a11af, group=Treatment))+
  geom_line(aes(color=Treatment))+
  geom_point(aes(color=Treatment))+
  ylim(-1,7.5)+
  ylab("Average CLR Abundance (M.Schaedleri)")+
...
schaed_4 %>%
  ggexport(filename = "schaed4.pdf")

allobaculum = ggplot(data = mean_var, aes(x=Time, y=X2e59f1b99fab85cb63bce2f35a5ca838, group=Treatment))+
  geom_line(aes(color=Treatment))+
  geom_point(aes(color=Treatment))+
  ylim(-1,7.5)+
  ylab("Average CLR Abundance (Allobaculum)")+
  xlab("Time (Week)")

allobaculum %>%
  ggexport(filename = "allobaculum.pdf")

lactobac_1 = ggplot(data = mean_var, aes(x=Time, y=X626d5d8df116add83e6b14c60adbc2dc, group=Treatment))+
  geom_line(aes(color=Treatment))+
  geom_point(aes(color=Treatment))+
  ylim(-1,7.5)+
  ylab("Average CLR Abundance (Lactobacillus_1)")+
  xlab("Time (Week)")

lactobac_1 %>%
  ggexport(filename = "lactobillus_1.pdf")

lactobac_2 = ggplot(data = mean_var, aes(x=Time, y=X64547667c88735701e168289a2a66718, group=Treatment))+
  geom_line(aes(color=Treatment))+
  geom_point(aes(color=Treatment))+
  ylim(-1,7.5)+
  ylab("Average CLR Abundance (Lactobacillus_2)")+
  xlab("Time (Week)")

lactobac_2 %>%
  ggexport(filename = "lactobacillus_2.pdf")
```
...
rf_class <- read.csv('2022-09-21_rf_importance.csv')
rf_class
...
#xformed %>% write_csv(paste(lubridate::today(), 'LMM_taxa_table_W_Weight.csv', sep = "_"))
```
